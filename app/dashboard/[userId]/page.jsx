...
export default function DashboardPage() {
  ...
  const [user, setUser] = useState({ role: "patient", id: null, name: "" }); // patient being viewed
  ...
  const handleAddReport = () => router.push("/reports/upload");
  ...
  // Delete medication (only patient themselves)
  ...
  const filteredMeds = useMemo(() => {
    return data.meds
      .filter((m) => {
        if (medFilters.status) {
          if (medFilters.status === "taken")
            return m.filteredDoses?.some((d) => d.taken === true);
          if (medFilters.status === "missed")
            return m.filteredDoses?.some((d) => d.taken === false);
        }
        return true;
      })
      .map((m) => {
          ...
          <div className="flex flex-col gap-2">
            ...
            <button
              ...
              className={`px-4 py-2 rounded transition ${
                loggedInUser.role === "doctor" ||
                (loggedInUser.role === "patient" && loggedInUser.id === userId)
            ...
            <button
              ...
              disabled={loggedInUser.role !== "doctor"}
              className={`px-4 py-2 rounded transition ${
                loggedInUser.role === "doctor"
                ...
                {filteredMeds.map((m) => (
                  <li key={m._id} className="border rounded p-3 bg-blue-50">
                    <div className="flex justify-between items-center">
                      ...
                      {/* DELETE BUTTON - only patient themselves */}
                      {loggedInUser.role === "patient" &&
                        loggedInUser.id === userId &&
                        (m.showDeleteConfirm ? (
